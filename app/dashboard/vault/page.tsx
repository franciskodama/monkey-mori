...
export default async function VaultPage(props: {
  ...
  const selectedNote = noteId && noteId !== 'new'
        ...
        <div className='w-full md:w-1/3 flex flex-col bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-3xl overflow-hidden shadow-2xl h-[calc(100vh-4rem)]'>
          <div className='p-6 border-b border-slate-800 flex items-center justify-between'>
            <div>
              ...
              <p className='text-sm text-slate-400 mt-1'>{notes.length} saved notes</p>
            </div>
            <Link href='/dashboard'>
          ...
          <div className='p-6 flex-1 overflow-y-auto space-y-4'>
            ...
            <div className='mt-6 space-y-3'>
              ...
              {notes.map((note) => (
                <Link key={note.id} href={`/dashboard/vault?id=${note.id}`} className='block'>
                  <div className={`p-4 rounded-xl border transition-all ${
                    noteId === note.id 
                      ? 'bg-indigo-500/10 border-indigo-500/30' 
                      : 'bg-slate-950 border-slate-800 hover:border-slate-700 hover:bg-slate-900'
                  }`}>
                    <h3 className='font-medium text-slate-200 line-clamp-1'>{note.title}</h3>
        ...
        <div className='flex-1 bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-3xl shadow-2xl h-[calc(100vh-4rem)] flex flex-col'>
          {(!isCreating && !isEditing) ? (
            <div className='flex-1 flex flex-col items-center justify-center p-8 text-center'>
              ...
              <h3 className='text-xl font-medium text-slate-300'>Select an entry</h3>
              <p className='text-sm text-slate-500 mt-2 max-w-sm'>
                Choose an existing note from the list or create a new one to store your important household directions.
              </p>
            </div>
          ) : (
            <div className='flex-1 flex flex-col overflow-y-auto'>
              <div className='p-8 border-b border-slate-800 flex items-center justify-between'>
                ...
                {isEditing && (
                  <form action={deleteNote}>
                    ...
                    <Button variant='outline' size='sm' className='border-red-500/20 text-red-400 hover:bg-red-500/10 hover:text-red-300'>
              ...
              <form action={saveNote} className='p-8 flex-1 flex flex-col gap-6'>
                <input type='hidden' name='id' value={selectedNote?.id || 'new'} />
                ...
                <div className='space-y-2'>
                  <Label htmlFor='visibility' className='text-slate-300'>Visibility level</Label>
                  <select 
                    id='visibility'
                    name='visibility'
                  ...
                  >
                    <option value='SHARED'>Shared (Visible to partner immediately)</option>
                    <option value='PRIVATE'>Private (Revealed only if switch triggers) - Coming Soon</option>
                  </select>
                </div>

                <div className='space-y-2 flex-1 flex flex-col'>
                  <Label htmlFor='content' className='text-slate-300'>Directions & Details</Label>
                  <Textarea 
                    id='content' 
                    name='content' 
                    required
